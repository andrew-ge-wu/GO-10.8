...
public class OColorPickerPolicyWidgetTest {
    ...
    @Test
    public void testInitSelfAndInitValueWhenValueIsSet() throws OrchidException, CMException, IOException {
        when(contentSession.getMode()).thenReturn(ContentSession.EDIT_MODE);
        target.initSelf(oc);
        target.initValueFromPolicy();

        assertEquals("#000000", target.getValue());
